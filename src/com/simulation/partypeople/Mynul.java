...
public class Mynul extends Avatar {
	...
	public Direction moveAvatar() {
		...
		if (isOnDanceFloor) {
			...
			dir = Direction.IDLE;
		} else if (goToLoungeArea && (surroundingsMap.getOrDefault("next", Places.PATH) == Places.LOUNGE_BIG
				|| surroundingsMap.getOrDefault("next", Places.PATH) == Places.LOUNGE_SMALL)) {
			if (!reachedAtLounge) {
	...
	private void updateMindMap() {
		updateMentalMap(this.getWhatISee(), this.myLocation.getHeading(), this.myLocation.getX(),
				this.myLocation.getY());
	}
